"""OKXQuant-native notification fan-out. No QwenPaw/OpenClaw runtime dependency."""
...
def _post_json(url: str, payload: dict[str, Any], headers: dict[str, str] | None = None) -> tuple[bool, str, dict[str, Any]]:
    request_headers = {"Content-Type": "application/json", "User-Agent": "OKXQuant-Standalone/0.1.0"}
...
def _send_qq(env: dict[str, str], message: str) -> tuple[bool, str]:
    app_id = env.get("OKXQUANT_QQ_APP_ID", "").strip()
    secret = env.get("OKXQUANT_QQ_CLIENT_SECRET", "").strip()
    openid = env.get("OKXQUANT_QQ_OPENID", "").strip()
...
def enabled_channels(env: dict[str, str] | None = None) -> list[str]:
    ...
    for channel in ("webhook", "wechat", "telegram", "qq"):
        if env.get(f"OKXQUANT_NOTIFY_{channel.upper()}_ENABLED") == "1":
...
def diagnose_channel(channel: str, env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    if missing:
        if channel == "qq" and missing == ["OKXQUANT_QQ_OPENID"]:
            return {
                "status": "incomplete",
                "missing": missing,
...
def send_channel(channel: str, message: str, env: dict[str, str] | None = None) -> tuple[bool, str]:
    env = env or _env()
    if channel == "webhook":
        url = env.get("OKXQUANT_NOTIFICATION_WEBHOOK", "").strip()
        if not url:
        ...
        if "dingtalk" in u_lower or "oapi.dingtalk.com" in u_lower:
            ...
            payload = {"title": "【OKXQuant 量化通知】", "desp": message}
        elif "pushdeer" in u_lower:
            payload = {"text": "【OKXQuant 量化通知】", "desp": message}
        else:
            payload = {
                "source": "OKXQuant",
                "message": message,
    ...
    if channel == "wechat":
        url = env.get("OKXQUANT_WECHAT_WEBHOOK", "").strip()
        if not url:
    ...
    if channel == "telegram":
        token = env.get("OKXQUANT_TELEGRAM_BOT_TOKEN", "").strip()
        chat_id = env.get("OKXQUANT_TELEGRAM_CHAT_ID", "").strip()
        tg_base = (env.get("OKXQUANT_TELEGRAM_API_BASE", "") or "https://api.telegram.org").strip().rstrip("/")
...
def test_channel(channel: str) -> dict[str, str]:
    ...
    ok, detail = send_channel(channel, f"【OKXQuant】{timestamp}\n🔔 {channel.upper()} 通知测试：指定通道连接正常。", env)
