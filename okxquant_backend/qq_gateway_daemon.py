...
3. Automatically saves OKXQUANT_QQ_OPENID to the encrypted secret store and .env.
                            ...
                            if openid:
                                ...
                                if not current_openid or content in ("/bind", "绑定", "bind", "重置绑定"):
                                    ...
                                    _send_ack(
                                        token,
                                        str(openid).strip(),
                                        "【OKXQuant】✅ 机器人已成功绑定您的 OpenID！量化交易、平仓与风险预警通知将实时推送到此会话。",
                                    )
                                elif content in ("ping", "Ping", "测试", "test"):
                                    _send_ack(token, str(openid).strip(), "【OKXQuant】🏓 Pong! 机器人通信链路正常。")
