...
def _get_qq_token(app_id: str, client_secret: str) -> tuple[bool, str, str]:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "OKXQuant-Standalone/0.1.0"},
        method="POST",
    )
    try:
...
def _get_gateway_ws_url(access_token: str) -> tuple[bool, str]:
    ...
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"QQBot {access_token}", "User-Agent": "OKXQuant-Standalone/6.2.0"},
    )
    try:
...
def _post_qq(path: str, payload: dict[str, Any], timeout: int = 12) -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        ...
        data=body,
        headers={"Content-Type": "application/json", "Accept": "application/json", "User-Agent": "OKXQuant-Standalone/6.2.0"},
        method="POST",
    )
...
def ensure_qq_gateway_daemon_running() -> None:
    ...
    try:
        ...
        if "okxquant_backend.qq_gateway_daemon" in output:
            return
        with open(log_file, "a", encoding="utf-8") as f:
            subprocess.Popen(
                [sys.executable, "-m", "okxquant_backend.qq_gateway_daemon"],
                cwd=root,
...
def create_bind_task(source: str = "OKXQuant") -> dict[str, Any]:
        ...
        async with websockets.connect(ws_url, close_timeout=5, ping_interval=None) as ws:
            ...
            auth_payload = {
                ...
                "d": {
                    ...
                    "intents": intents,
                    "shard": [0, 1],
                    "properties": {"$os": "linux", "$browser": "okxquant", "$device": "okxquant"},
                },
            }
                        ...
                        if openid:
                            ...
                            try:
                                ...
                                ack_req = urllib.request.Request(
                                    ...
                                    data=json.dumps({
                                        "content": "【OKXQuant】✅ OpenID 自动捕获并绑定成功！此账号已设为交易通知接收目标。",
                                        "msg_type": 0,
                                        "msg_seq": seq_num,
                                    }).encode("utf-8"),
                                    headers={
...
def start_openid_capture(app_id: Optional[str] = None, client_secret: Optional[str] = None, timeout: int = 60) -> dict[str, Any]:
    ...
    from okxquant_backend.notifications import _env
    env = _env()
    effective_app_id = (app_id or env.get("OKXQUANT_QQ_APP_ID", "")).strip()
    effective_secret = (client_secret or env.get("OKXQUANT_QQ_CLIENT_SECRET", "")).strip()
