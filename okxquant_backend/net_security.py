...
def validate_outbound_url(url: str, *, allow_private: bool = False, allowed_hosts: set[str] | None = None) -> str:
    ...
    if not addresses:
        raise ValueError("Endpoint has no resolved addresses")
...
def validate_oauth_verification_uri(uri: str) -> str:
    """Validate a link displayed to the operator; never resolve or visit it."""
    value = str(uri).strip()
    if any(ord(c) < 32 or ord(c) == 127 for c in value) or "\\" in value:
        raise ValueError("Official OAuth verification URL is invalid")
    parsed = urllib.parse.urlsplit(value)
    host = parsed.hostname or ""
    if (parsed.scheme != "https" or parsed.port not in (None, 443)
            or parsed.username or parsed.password
            or not (host in {"okx.com", "okx.us", "okx.com.tr"}
                    or host.endswith((".okx.com", ".okx.us", ".okx.com.tr")))):
        raise ValueError("Official OAuth verification URL is invalid")
    return value
