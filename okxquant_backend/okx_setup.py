"""Local OKX CLI/OAuth dependency diagnostics for standalone OKXQuant deployments."""
...
def diagnose_okx_runtime(selected_mode: str, static_configured: bool, *, env: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    if not binary:
        ...
        status["steps"].extend(["安装 Node.js 18+ 与 npm", f"执行：{INSTALL_COMMAND}", "安装后重启 okxquant-backend 与 okxquant-gateway"])
        return status
    ...
    if status["cli"]["supported"] and (static_configured or oauth_ready or status["api_key_profiles"]):
        ...
        # changes OKXQuant's selected environment and never submits an order.
    ...
    if not static_configured and not oauth_ready and not status["api_key_profiles"]:
        ...
        status["steps"].extend([
            "在部署 OKXQuant 的同一 Linux 用户下执行 OAuth 登录，或在后台填写对应环境 API Key",
