...
def bar_seconds(bar):
    ...
    if not match or int(match[1])<=0: raise SignalDataError('Unsupported signal interval')
...
def _timestamp(value):
    try:
        ...
        parsed=int(value)
        numeric=float(value)
        if not math.isfinite(numeric) or parsed<0 or numeric!=parsed: raise ValueError()
        return parsed
    except (ValueError,TypeError,OverflowError):
        raise SignalDataError('Invalid candle timestamp') from None


def closed_candles(rows, bar, *, as_of_ms=None, limit=None, require_fresh=True):
    as_of_ms=int(time.time()*1000) if as_of_ms is None else _timestamp(as_of_ms)
    ...
    for row in rows:
        ...
        try:
            if any(isinstance(row[i],bool) for i in (1,2,3,4,5)): raise ValueError()
            values=[float(row[i]) for i in (1,2,3,4,5)]
        except (ValueError,TypeError,OverflowError):
            raise SignalDataError('Invalid closed candle') from None
