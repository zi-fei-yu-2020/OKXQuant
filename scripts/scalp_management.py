...
def adopt(tracker,saved,position,scope):
    ...
    if (not isinstance(ctx,dict) or ctx.get('version')!=VERSION or ctx.get('scope')!=scope
...
def evaluate(tracker,factor,*,entry,current,side,now,policy,tick):
    ...
    if ctx.get('version')!=VERSION or setup not in SETUPS or ctx.get('side')!=side:return inactive
    ...
    if peak>=activation:
        ...
        retained=max(costs,peak*(.65 if tier==2 else .5))
        desired=entry+sign*retained;crossed=sign*(current-desired)<=0
        # Never widen/erase a floor activated on an earlier observation.
        ...
        if crossed or sign*(current-desired)>=buffer:
            protection={'active':True,'kinetic_exit':False,'stop':desired,'crossed':crossed,
            ...
            stage='PROFIT_LOCK' if tier==2 else 'COST_PROTECTED'
    result={'version':VERSION,'enabled':True,'state':stage,'exit':False,'reason':'structure_pending',
            'setup':setup,'alignment':ctx.get('alignment'),'peak_gain':peak,'profit_r':gain/risk,
    ...
    try:
        ...
        if broken and current_broken and gain<0 and peak<risk*.7:
            result.update(state='FAILED',exit=True,reason='two_closed_bars_reentered_invalidated_structure',
