...
class QqBindTests(unittest.TestCase):
    ...
    def test_create_task_builds_official_connect_url(self):
        with patch("okxquant_backend.qq_bind._post_qq", return_value={"task_id": "t-123"}) as post:
    ...
    def test_poll_bound_persists_credentials_without_plaintext_leak(self):
        with patch("okxquant_backend.qq_bind._post_qq", return_value={"task_id": "t-9"}):
        ...
        with patch("okxquant_backend.qq_bind._post_qq", side_effect=lambda path, payload, timeout=12: next(responses)), \
             patch("okxquant_backend.qq_bind._persist") as persist:
    ...
    def test_poll_bound_without_openid_transitions_to_awaiting_message(self):
        with patch("okxquant_backend.qq_bind._post_qq", return_value={"task_id": "t-await"}):
            qq_bind.create_bind_task()
        ...
        with patch("okxquant_backend.qq_bind._post_qq", side_effect=lambda path, payload, timeout=12: next(responses)), \
             patch("okxquant_backend.qq_bind._persist") as persist, \
             patch("okxquant_backend.qq_bind.start_openid_capture", return_value={"capture_id": "cap-auto-1"}) as start_cap:
    ...
    def test_poll_expired_task_marks_expired(self):
        with patch("okxquant_backend.qq_bind._post_qq", return_value={"task_id": "t-e"}):
            qq_bind.create_bind_task()
        task = qq_bind._TASKS["t-e"]
        task.created_at -= qq_bind.TASK_TTL_SECONDS + 5
        with patch("okxquant_backend.qq_bind._post_qq") as post:
    ...
    def test_persist_writes_secret_store_and_env_only(self):
        with patch("okxquant_gateway.secrets.save_secrets") as save, patch("okxquant_backend.settings_store.update_env") as env:
            qq_bind._persist("100", "sekret", "OPEN")
        save.assert_called_once_with({"OKXQUANT_QQ_CLIENT_SECRET": "sekret", "OKXQUANT_QQ_OPENID": "OPEN"})
        env.assert_called_once_with({"OKXQUANT_QQ_APP_ID": "100", "OKXQUANT_QQ_OPENID": "OPEN"})

    def test_active_task_cap(self):
        counter = iter(f"task-{i}" for i in range(10))
        with patch("okxquant_backend.qq_bind._post_qq", side_effect=lambda path, payload, timeout=12: {"task_id": next(counter)}):
