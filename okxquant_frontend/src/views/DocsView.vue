    ...
    <main class="terminal-main docs-main">
      ...
      <div class="docs-layout">
        ...
        >
          ...
          <p class="text-xs sm:text-sm leading-relaxed font-sans" style="color: var(--text-muted)">
            OKXQuant 的自进化是<strong>review-only 复盘与候选审核流程</strong>：按配置时间运行，
          ...
          <p class="text-sm leading-relaxed" style="color:var(--text-muted)">策略复盘默认展示核心报告、复盘样本、净费用后胜率与待审核数量；已发布运行记忆单独展示。净费用后胜率仅用当前账户核验的结构化证据计算，盈亏平衡计入已结算分母；旧报告自报胜率保留在任务详情中，不冒充已核验净胜率。长篇决策与等待审计默认折叠，完整原文与证据入口仍可展开。</p>
